...
namespace Poderosa.Terminal {
	...
	/// <summary>
	/// DocumentPane ÇÃäTóvÇÃê‡ñæÇ≈Ç∑ÅB
	/// </summary>
	public class TerminalPane : UserControl, IPoderosaTerminalPane, IInternalTerminalPane {
		...
		protected override sealed void OnPaint(PaintEventArgs e) {
			...
			try {
				...
				*/
				...
				if(param.LineFrom <= document.LastLineNumber) {
					...
					try {
						if(prof.UseClearType) {
							...
							Win32.GdipSetTextRenderingHint(gdipgraphics, 5); //5ÇÕTextRenderingHintClearTypeGridFit
						}
				...
				if(!_criticalErrorRaised) { //Ç±ÇÃíÜÇ≈àÍìxó·äOÇ™î≠ê∂Ç∑ÇÈÇ∆åJÇËï‘ÇµãNÇ±Ç¡ÇƒÇµÇ‹Ç§Ç±Ç∆Ç™Ç‹Ç‹Ç†ÇÈÅBÇ»ÇÃÇ≈èââÒÇÃÇ›ï\é¶ÇµÇƒÇ∆ÇËÇ†Ç¶Ç∏êÿÇËî≤ÇØÇÈ
		...
		private void BuildTransientDocument(PaintEventArgs e, ArrayList lines, ref RenderParameter param, ref bool caret, ref int caret_pos_x, ref int caret_pos_y) {
			if(_tag == null) return;	// ÉyÉCÉìÇï¬Ç∂ÇÈéûÇ… _tag Ç™ null Ç…Ç»Ç¡ÇƒÇ¢ÇÈÇ±Ç∆Ç™Ç†ÇÈ
			...
			param.TargetRect = new Rectangle(sm.ControlBorderWidth+1, sm.ControlBorderHeight,
				this.Width - _VScrollBar.Width - sm.ControlBorderWidth + 8, //Ç±ÇÃÇWÇ™Ç»Ç¢ílÇ™ê≥ìñÇæÇ™ÅA.NETÇÃï∂éöÉTÉCÉYä€Çﬂñ‚ëËÇÃÇΩÇﬂçsÇÃç≈èIï∂éöÇ™ï\é¶Ç≥ÇÍÇ»Ç¢Ç±Ç∆Ç™Ç†ÇÈÅBÇ±ÇÍÇâÒîÇ∑ÇÈÇΩÇﬂÇ…ÇøÇÂÇ¡Ç∆ëùÇ‚Ç∑
			...
			if(selection.Owner==this && !selection.IsEmpty) {
				...
				if(l!=null && t!=null) { //ñ{ìñÇÕlÇ™nullÇ≈ÇÕÇ¢ÇØÇ»Ç¢ÇÕÇ∏ÇæÇ™ÅAÇªÇÍÇé¶ç¥Ç∑ÇÈÉoÉOÉåÉ|Å[ÉgÇ™Ç†Ç¡ÇΩÇÃÇ≈îOÇÃÇΩÇﬂ
					t = t.NextLine;
					int pos = from._position; //ÇΩÇ∆Ç¶ÇŒç∂í[ÇâzÇ¶ÇƒÉhÉâÉbÉOÇµÇΩÇ∆Ç´ÇÃëIëîÕàÕÇÕëOçsññÇ…Ç»ÇÈÇÃÇ≈ pos==TerminalWidthÇ∆Ç»ÇÈÉPÅ[ÉXÇ™Ç†ÇÈÅB
					do {
						...
						pos = 0; //ÇQçsñ⁄Ç©ÇÁÇÃëIëÇÕçsì™Ç©ÇÁ
						l = l.NextLine;
					} while(l!=t);
				}
			}

			//ÉLÉÉÉåÉbÉgà íuÇÃåvéZ
			...
			if(_selectionKeyProcessor!=null) {
				...
				if(_caretState==1) caret = false; //ÉeÉLÉXÉgëIëÉÇÅ[ÉhÇ≈ÇÕèÌÇ…ÉuÉäÉìÉN
			}
			else {
			...
			//CaretâÊñ äOÇ…Ç†ÇÈÇ»ÇÁèàóùÇÕÇµÇ»Ç≠ÇƒÇÊÇ¢ÅBÇQî‘ñ⁄ÇÃèåèÇÕÅAAttach-ResizeTerminalÇÃó¨ÇÍÇÃíÜÇ≈Ç±ÇÃOnPaintÇé¿çsÇµÇΩèÍçáÇ…TerminalHeight>lines.CountÇ…Ç»ÇÈÉPÅ[ÉXÇ™Ç†ÇÈÇÃÇñhé~Ç∑ÇÈÇΩÇﬂ
			if(caret_line!=null) { 
				//ÉqÉNÉqÉNñ‚ëËÇÃÇΩÇﬂÅAÉLÉÉÉåÉbÉgÇï\é¶ÇµÇ»Ç¢Ç∆Ç´Ç≈Ç‡Ç±ÇÃëÄçÏÇÕè»ÇØÇ»Ç¢
        ...
        protected void _VScrollBar_ValueChanged(object sender, System.EventArgs e) {
			...
			TerminalDocument document = _tag.Document;
			lock(document) {
				//if(_tag.Terminal.TerminalMode==TerminalMode.Normal) Ç±ÇÃifï∂ÇÃà”ê}Ç¢Ç‹Ç¢Çøïsñæ
        ...
        private void ProcessSpecialAltKey(AltKeyAction act, Keys modifiers, Keys body) {
			...
			if(ch=='\0') return; //äÑÇËìñÇƒÇÁÇÍÇƒÇ¢Ç»Ç¢Ç‚Ç¬ÇÕñ≥éã

			if((modifiers & Keys.Control)!=Keys.None)
				ch = (char)((int)ch % 32); //ControlÇâüÇµÇΩÇÁêßå‰ï∂éö
        ...
        private void OnKeyPress(object sender, KeyPressEventArgs e) {
			...
			if(ch=='\r') {
				...
				if(ch==' ' && Control.ModifierKeys==Keys.Control) ch = '\0'; //Ctrl+SpaceÇ≈NULëóêM
		...
		public void MakeCurrentLineVisible() {
			...
			else if(_VScrollBar.Value + GetConnection().TerminalHeight <= document.CurrentLineNumber-document.FirstLineNumber) { //â∫Ç…âBÇÍÇΩ
		...
		protected override void OnResize(EventArgs args) {
			base.OnResize(args);
			Invalidate();
			//ç≈è¨âªéûÇ…ÇÕÇ»Ç∫Ç©é©êgÇÃïùÇæÇØÇ™ÇOÇ…Ç»Ç¡ÇƒÇµÇ‹Ç§
		...
		private void OnCaretTimer(object sender, EventArgs args) {
			...
			//if(this.Focused) Invalidate(); //ì_ñ≈Ç≈InvalidateÇ∑ÇÈÇÃÇÕÉtÉHÅ[ÉJÉXÇ™Ç†ÇÈÇ∆Ç´ÇæÇØÇ≈ÇÊÇ¢ÇÕÇ∏ÇæÇ™ÅAFocusedÇ™Ç®Ç©ÇµÇ»ílÇï‘ÇµÇƒÇ¢ÇÈÇÁÇµÇ≠ã@î\ÇµÇ»Ç¢Ç±Ç∆Ç™Ç†ÇÈ
		}
		...
		protected override void OnMouseWheel(MouseEventArgs e) {
			if(_tag!=null && !GEnv.Options.AllowsScrollInAppMode && _tag.Terminal.TerminalMode==TerminalMode.Application && _selectionKeyProcessor==null) { //ÉAÉvÉäÉPÅ[ÉVÉáÉìÉÇÅ[ÉhÇ≈ÇÕí èÌèàóùÇÇ‚ÇﬂÇƒÉJÅ[É\Éãè„â∫Ç∆ìØìôÇÃèàóùÇ…Ç∑ÇÈ
		...
		protected override void OnMouseDown(MouseEventArgs args) {
			...
			lock(document) {
				...
				if(sel.Owner!=this || sel.State==SelectionState.Fixed) sel.Clear(); //ïœÇ»Ç∆Ç±ÇÎÇ≈MouseDownÇµÇΩÇ∆ÇµÇƒÇ‡ClearÇæÇØÇÕÇ∑ÇÈ
				if(target_id <= document.LastLineNumber) {
					...
					//É}ÉEÉXÇìÆÇ©ÇµÇƒÇ¢Ç»Ç≠ÇƒÇ‡ÅAMouseDownÇ∆Ç∆Ç‡Ç…MouseMoveÇ™óàÇƒÇµÇ‹Ç§ÇÊÇ§Çæ
		...
		protected override void OnGotFocus(EventArgs args) {
			...
			if(_tag!=null) { //èâä˙âªâﬂíˆÇÃÇ∆Ç´ÇÕñ≥éã
				//.NET1.1 SP1 issue
				GEnv.Frame.ActivateConnection(_tag);
				if(!_caretTimer.Enabled) _caretTimer.Start(); //ÉXÉvÉäÉbÉ^ÇÃÉhÉâÉbÉOÇÉLÉÉÉìÉZÉãÇµÇΩå„ÇÕÉäÉTÉCÉYÇ‡Ç≥ÇÍÇ»Ç¢ÇÃÇ≈Ç±ÇÃÉ^ÉCÉ~ÉìÉOÇ≈ÉLÉÉÉåÉbÉgÇçƒâ“ìÆÇ∑ÇÈ
		...
		protected override void WndProc(ref Message msg) {
			...
			if(msg.Msg==Win32.WM_VSCROLL) {
				...
				if(msg.LParam==IntPtr.Zero && _VScrollBar.Enabled)
					ProcessVScrollMessage(msg.WParam.ToInt32()); //ä¬ã´Ç…ÇÊÇ¡ÇƒÇÕWHEELÇÃÇ©ÇÌÇËÇ…VSCROLLÇ™óàÇÈÇ∆évÇÌÇÍÇÈ
			}
			else if(msg.Msg==Win32.WM_IME_STARTCOMPOSITION) {
				_inIMEComposition = true; //_inIMECompositionÇÕWM_IME_STARTCOMPOSITIONÇ≈ÇµÇ©ÉZÉbÉgÇµÇ»Ç¢
				...
				switch(msg.WParam.ToInt32()) {
					case GConst.WPG_ADJUSTIMECOMPOSITION:
						if(_inIMEComposition) AdjustIMEComposition(0); //ïîï™ämíËÇµÇΩÇ∆Ç´Ç…ëŒâûÅBDataArrivedÇÃéûì_Ç≈ÇÕëSämíËÇ©ïîï™ämíËÇ©ÇÌÇ©ÇÁÇ»Ç¢ÇÃÇ≈Ç«ÇÃÇ›ÇøÇ±Ç±Ç÷Ç≠ÇÈ
						break;
